...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, pbr_kwargs=None):
    ...
    if gaussians.use_pbr:
        ...
        makedirs(indirect_path, exist_ok=True)
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        mask = view.image_mask.cuda()
        save_image(torch.cat((results["render"], mask), dim=0), os.path.join(render_path, '{0:05d}'.format(idx) + ".png"))
        save_image(torch.cat((gt, mask), dim=0), os.path.join(gts_path, '{0:05d}'.format(idx) + ".png"))
        save_image(torch.cat(((results["normal"] * 0.5 + 0.5), mask), dim=0), os.path.join(normal_path, '{0:05d}'.format(idx) + ".png"))

        # save_image(gt * mask, os.path.join(gts_path, '{0:05d}'.format(idx) + ".png"))
        # save_image((results["normal"] * 0.5 + 0.5) * mask,  os.path.join(normal_path, '{0:05d}'.format(idx) + ".png"))
        ...
        if gaussians.use_pbr:
            ...
            save_image(torch.cat((results["indirect"], mask), dim=0), os.path.join(indirect_path, '{0:05d}'.format(idx) + ".png"))
...
def render_sets(dataset : ModelParams, pipeline : PipelineParams, skip_train : bool, skip_test : bool):
    with torch.no_grad():
        ...
        if iteration is not None and gaussians.use_pbr:
            ...
            gaussians.update_radiace(args.sample_num)
        ...
        if not skip_train:
             render_set(dataset.model_path, "train", iteration, scene.getTrainCameras(4), gaussians, pipeline, background, pbr_kwargs)

        if not skip_test:
             render_set(dataset.model_path, "test", iteration, scene.getTestCameras(4), gaussians, pipeline, background, pbr_kwargs)
