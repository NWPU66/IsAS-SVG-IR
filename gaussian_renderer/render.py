...
def render_view(camera: Camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, 
    ...
    (num_rendered, num_contrib, rendered_image, rendered_normal_, rendered_opacity, rendered_depth,
    ...
    rendered_pseudo_normal = depth2normal(rendered_depth, camera.image_mask.cuda(), camera)
    
    rendered_normal, rendered_depth_, rendered_depth2 = torch.split(rendered_feature, [3, 1, 1], dim=0)
...
def calculate_loss(viewpoint_camera, pc, render_pkg, opt, iteration):
    ...
    loss += 0.01 * loss_mask
    loss += (0.01 + 0.01 * min(iteration / opt.iterations, 1)) * loss_surface

    if mono is not None:
        mono = torch.from_numpy(mono).cuda().float()
        mono *= image_mask
        monoN = mono[:3]

        loss_monoN = cos_loss(rendered_normal, monoN, weight=image_mask)
