...
def calculate_loss(viewpoint_camera, pc, results, opt, direct_light_env_light, iteration):
    ...
    loss_per_pixel = loss_per_pixel + torch.ones_like(loss_per_pixel) * loss_radiance
    ...
    if opt.lambda_light > 0:
        ...
        loss_light_per_pixel = torch.abs(diffuse_light - mean_light)
        tb_dict["loss_light"] = loss_light.item()
        loss = loss + opt.lambda_light * loss_light
        loss_per_pixel = loss_per_pixel + opt.lambda_light * loss_light_per_pixel

    if opt.lambda_base_color_smooth > 0:
        image_mask = viewpoint_camera.image_mask.cuda()
        dict_params = {}
        loss_base_color_smooth = first_order_edge_aware_loss(rendered_base_color * image_mask, gt_image * image_mask, dict_params)
        ...
        loss_per_pixel = loss_per_pixel + opt.lambda_base_color_smooth * dict_params["loss_per_pixel"]

    if opt.lambda_roughness_smooth > 0:
        image_mask = viewpoint_camera.image_mask.cuda()
        dict_params = {}
        loss_roughness_smooth = first_order_edge_aware_loss(rendered_roughness * image_mask, gt_image * image_mask, dict_params)
        ...
        loss_per_pixel = loss_per_pixel + opt.lambda_roughness_smooth * dict_params["loss_per_pixel"]
    
    if opt.lambda_metallic_smooth > 0:
        image_mask = viewpoint_camera.image_mask.cuda()
        dict_params = {}
        loss_metallic_smooth = first_order_edge_aware_loss(rendered_metallic * image_mask, gt_image * image_mask, dict_params)
        tb_dict["loss_metallic_smooth"] = loss_metallic_smooth.item()
        loss = loss + opt.lambda_metallic_smooth * loss_metallic_smooth
        loss_per_pixel = loss_per_pixel + opt.lambda_metallic_smooth * dict_params["loss_per_pixel"]
    
    if opt.lambda_light_smooth > 0:
        image_mask = viewpoint_camera.image_mask.cuda()
        dict_params = {}
        loss_light_smooth = first_order_edge_aware_loss(rendered_diffuse * image_mask, rendered_normal, dict_params)
        ...
        loss_per_pixel = loss_per_pixel + opt.lambda_light_smooth * dict_params["loss_per_pixel"]
    ...
    if opt.lambda_env_smooth > 0:
        ...
        loss_per_pixel = loss_per_pixel + torch.ones_like(loss_per_pixel) * opt.lambda_env_smooth * loss_env_smooth
...
def render_IsAS_svgss(viewpoint_camera: Camera, pc: GaussianModel, pipe, bg_color: torch.Tensor,
    ...
    if is_training:
        _, tb_dict = calculate_loss(viewpoint_camera, pc, results, opt, direct_light_env_light=dict_params['env_light'], iteration=iteration)
        results["tb_dict"] = tb_dict
        loss_per_pixel = results["loss_per_pixel"]  # C, W, H

        """ 
        ================================================================================
        Image Space Adaptive Sampling Stage 2:

        rerender image at full resolution.
        calculate d(loss)/d(theta) per pixel multiply by 1/pj.
        sum them up and backpropagate for gradient.
        """
        pj = upsampling_half_pj(half_pj)    # C, W, H
        loss_per_pixel = loss_per_pixel / torch.clamp_min(pj, 1e-8)
        results["loss"] = torch.sum(loss_per_pixel)
